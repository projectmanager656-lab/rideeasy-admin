  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-[#152238] sm:text-3xl">
            Dashboard
          </h1>
        ...
        <button
          type="button"
          onClick={onRefresh}
          className="hidden w-fit items-center gap-2 rounded-xl bg-[#FFB000] px-4 py-2.5 text-sm font-semibold text-[#071A2B] shadow-sm transition hover:bg-[#FFC34D] sm:inline-flex"
        >
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">
        {stats.map((stat) => (
          <div
            key={stat.title}
            className="rounded-xl border border-[#E6EBF2] bg-white p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:rounded-2xl sm:p-5"
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[11px] font-medium text-[#718096] sm:text-sm">
                  {stat.title}
                </p>

                <p className="mt-0.5 text-xl font-bold tracking-tight text-[#152238] sm:mt-2 sm:text-2xl">
                  {stat.value}
                </p>
              </div>

              <div
                className={`grid h-9 w-9 place-items-center rounded-lg ${stat.iconBg} ${stat.iconColor} sm:h-11 sm:w-11 sm:rounded-xl`}
              >
                <i className={`${stat.icon} text-xl`} />
              </div>
            </div>

            <p className="mt-1.5 text-[10px] text-[#718096] sm:mt-4 sm:text-xs">
              {stat.description}
            </p>
          </div>
        ))}
